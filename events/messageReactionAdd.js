...
module.exports = async (client, messageReaction, user) => {
  ...
  roles.forEach(role => {
    messageReaction.message.guild.members.cache.get(user.id).roles.add(role.role).then(thenBlock => {}) 
    .catch(err => messageReaction.message.channel.send(new MessageEmbed({
      color: "#FF0000",
      title: ":x: ERREUR :x:",
      description: `Impossible d'ajouter <@&${role.role}> à ${user} : permissions manquantes.`
    })))
  })
}